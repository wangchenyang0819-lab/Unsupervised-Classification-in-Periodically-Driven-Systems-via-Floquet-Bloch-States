function combinedPlot()
    ...
    function plotPhaseDiagram(ax, score, e)
        ...
        h1 =plot(ax, x_vals, pc1_centered, 'o', 'Color', color4, ...
             'MarkerSize', 6, 'LineStyle', 'none', 'LineWidth', 1.5, 'MarkerFaceColor', color4);
        hold on
        h2 =plot(ax, x_vals, pc2_centered, 's', 'Color', color5, ...
             'MarkerSize', 6, 'LineStyle', 'none', 'LineWidth', 1.5, 'MarkerFaceColor', color5);
legend(ax, [h1, h2], {'PC1-centered', 'PC2-centered'}, 'Location', 'best', 'FontSize', 10, 'Interpreter', 'latex','Position', [0.78, 0.6, 0.1, 0.05]);
        % Set axis properties
        ...
        set(ax, 'YTick', [-1 -0.5 0 0.5 1], 'FontSize', 18);
        xlabel(ax, '$JT/\pi$', 'Interpreter', 'latex', 'FontSize', 20, 'Color', 'k');
ylabel(ax, '$\mathrm{PC\ Value}$', 'Interpreter', 'latex', 'FontName', 'Times New Roman', 'FontSize', 20, 'Color', 'k');
        grid(ax, 'on');
        hold(ax, 'off');
